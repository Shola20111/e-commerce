...
export default function BestProducts() {
  return (
    <div className='w-full h-[350px] flex justify-between'>
      {bestSellingList.map((item)=>
      <div
      key={item.id}
      className='w-[250px]h-[350px] flex flex-col justify-between items-center items-start '
      >
        <div className='w-[250px] h-[250px] bg-[#F5F5F5] flex justify-center items-center rounded-[4px] relative'>
          <Image 
          src={item.image}
          width={140} height={140} alt='image'
          />
        ...
        <div className='h-[84px] w-auto flex flex-col'>
          <div className='text-[16px] leading-[24px] font-bold '>
            {item.name}
          </div>
          <div className='flex text-[16px] leading-[24px] gap-3 '>
            <div className='text-[#DB4444]'>${item.price}</div><div className='text-black opacity-50 line-through'>${item.oldPrice}</div>
          </div>
          <StarsComp/>
